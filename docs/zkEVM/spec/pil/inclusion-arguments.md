...
In other words, if one thinks of $\texttt{a}$ and $\texttt{b}$ as multisets and reduces them to sets (by removing the multiplicity), then $\texttt{a}$ is contained in $\texttt{b}$ if $\texttt{a}$ is a subset of $\texttt{b}$.
...
In the PIL context, the implemented inclusion argument is the same as the $\text{Plookup}$ method provided in [GW20](https://eprint.iacr.org/2020/315.pdf). Other "alternative" method exists such as the $\text{PlonkUp}$ described in [PFM+22](https://eprint.iacr.org/2022/086.pdf).
