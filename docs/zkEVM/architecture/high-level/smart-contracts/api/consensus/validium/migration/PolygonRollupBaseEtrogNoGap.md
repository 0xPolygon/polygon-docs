...
!!! note
    ...
    - In order to assure that users force transactions will be processed properly, users must not sign any other transaction with the same nonce.
...
Allow the admin to change the force batch address that is allowed to force batches. If address 0 is set, then everyone is able to force batches. 
